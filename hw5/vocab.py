...
class VocabEntry(object):
    ...
    def add(self, word):
        ...
        word not in self depend __contains__
        len(self) depend __len__
        self[word] depend __getitem__ iterate
        """
        if word not in self:
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var=pad_sents_char(sents,self.char_pad)
        sents_var=torch.tensor(sents_var,device=device).permute(1,0,2).contiguous()
        return sents_var
        ### END YOUR CODE
