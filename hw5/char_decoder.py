...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        h_t,dec_hidden=self.charDecoder(x,dec_hidden)  # sequence_leg,batch,num_direction*hidden_size
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        scores,dec_hidden=self.forward(x_input,dec_hidden)
        ...
        loss_char_dec=loss(scores,x_target)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        ###        Their indices are self.target_vocab.start_of_word and self.target_vocab.end_of_word,not sentence,respectively.
        ...
        for i in range(max_length):
            if i==0:
                char_in=torch.empty(1,batch_size,device=device,dtype=torch.long).fill_(self.target_vocab.start_of_word)
                scores,dec_hidden=self.forward(char_in,dec_hidden)
                char_out=torch.argmax(scores,dim=-1)
            else:
                scores, dec_hidden = self.forward(char_in, dec_hidden)
                char_out=torch.argmax(scores,dim=-1)
            char_in=char_out
            decodeChars.append([self.target_vocab.id2char[char_idx.item()]for char_idx in char_out[0]])

        for batch_id in range(batch_size):
            word=''
            for i in range(max_length):
                if self.target_vocab.char2id[decodeChars[i][batch_id]]!=self.target_vocab.end_of_word:
                    word=word+decodeChars[i][batch_id]
                else:
                    break
